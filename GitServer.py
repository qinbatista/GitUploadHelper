...
def change_shall_premssion(name):
    ...
    with open("/etc/passwd", mode='r', encoding="utf8") as file:
        ...
        for raw in content:
            if raw.find(name) != -1:
                new_content.append(raw[:raw.rfind(":")]+":/usr/bin/git-shell")
            else:
...
def add_ssh_to_user(name, key):
    if os.path.exists("/home/" + name + "/.ssh/") == False:
        os.system("mkdir "+"/home/" + name + "/.ssh/")
    if os.path.exists("/home/" + name + "/.ssh/") == False:
        os.system("touch authorized_keys")
...
def add_repositories():
    ...
    if os.path.isfile(file_name):
        ...
        for folder in folder_list:
            ...
            os.system('chmod 700 /GitRepositories/' + name+"/")
            print(f'[success]\tgit clone ssh://{name}@{my_domain_name}:{my_port}/GitRepositories/{name}/{folder}.git')
...
def main():
    while True:
        ...
        print("[6]\tCreate repositories from repositories.txt")
